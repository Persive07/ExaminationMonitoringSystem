<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        if(isset($_POST['Form'])){
            //Checking if the professor has already been registered
            $ProfID=$_POST['ProfID'];
            $sql1="SELECT *
                   FROM Prof_Records
                   WHERE ProfID='$ProfID'";
            ...
            if($result1->num_rows==0){
                ...
                if($result2->num_rows==0){
                    ...
                    if(password_verify($ConfirmPassword, $Password)){
                        ...
                        $sql3="INSERT INTO `Prof_Records`(`ProfID`,`FirstName`,`LastName`,`PhoneNo`,`Email`,`Department`,`ExamsConducting`,`Username`,`Password`)
                               VALUES ('$ProfID','$FirstName','$LastName','$PhoneNo','$Email','$Department',0,'$Username','$Password')";
                        $result3=$conn->query($sql3);
                        $status=3; //echo "Successfully Registered";
                        $_SESSION['ProfStatus']=$status;
                ...
                $status=3; //echo "The Professor has already been registered before";
            }
        }
    }
    $conn->close();
    ...
    <div id="prdiv">
        ...
        <?php
            if($status==0){
                ...
                echo "<p style='font-style:oblique; color:red; text-align:center;'>The Professor has already been registered before</p>";
            }
        ?>
        <form action="" method="post">
            ...
            <input type="text" name="ProfID" placeholder="Professor ID" required value="<?php if($status==2 || $status==3)echo isset($_POST['ProfID']) ? htmlspecialchars($_POST['ProfID']) : ''; ?>">
            ...
            <input type="text" name="Department" placeholder="Department" required value="<?php if($status==2 || $status==3)echo isset($_POST['Department']) ? htmlspecialchars($_POST['Department']) : ''; ?>">            

<?php
    ...
    //For updating HTML
    //$_SESSION['ProfStatus']=3 , Successfully Registered
    if(!isset($_SESSION['ProfStatus'])){
        $status=0;
    }else{
        $status=$_SESSION['ProfStatus'];
        unset($_SESSION['ProfStatus']);
    }
...
<body>
    <div class="logdiv">
        ...
        <h2 style="text-align:center;">Professor Login</h2>
        <?php
            if($status==0){
        ...
        <form action="" method="post">
            <input type="text" name="Username" placeholder="Username ( Professor )" required value="<?php if($status==1)echo isset($_POST['Username']) ? htmlspecialchars($_POST['Username']) : ''; ?>"><br> 
            <input type="password" name="Password" placeholder="Password ( Professor )" required><br> <br>
            <input type="submit" id="login_button" name="Login" value="Login"><br> 
            <p class="note">Don't have an account ? <a href="prof_reg.php" class="link">Register</a> <br><br>
            Student ?  <a href="index.php" class="link">Student Portal</a> <br><br>
            Administrator ?  <a href="admin_login.php" class="link">Admin Portal</a> </p>
        </form>
    </div>
</body>
</html>